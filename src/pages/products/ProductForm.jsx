...
import { Form, Input, Button, Select, Row, Col, message, Modal, Divider } from 'antd';
import { ShoppingOutlined, FileTextOutlined, PictureOutlined, AppstoreOutlined, PlusOutlined } from '@ant-design/icons';
import { fetchProduct, createProduct, updateProduct, clearCurrentProduct } from '@/store/productsSlice';
...
export default function ProductForm() {
  ...
  const { currentProduct, loading, creating, updating } = useSelector((state) => state.products);
  ...
  const loadCategories = async () => {
    ...
    try {
      ...
      raw.forEach((cat) => {
        flat.push(cat);
        if (cat.children?.length) cat.children.forEach((child) => flat.push(child));
      });
      setCategories(flat);
    } catch (err) {
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <FormSection icon={<ShoppingOutlined />} title={t('navigation.products')}>
          <Row gutter={[24, 0]}>
            <Col xs={24} md={12}>
              <Form.Item
              ...
              >
                <Input size="large" placeholder={t('products.titlePlaceholder')} />
              </Form.Item>
            </Col>
            <Col xs={24} md={12}>
              <Form.Item
                ...
                rules={[{ required: true, message: t('validation.required') }]}
              >
                <Input size="large" dir="rtl" placeholder={t('products.titleArPlaceholder')} />
              </Form.Item>
            </Col>
            <Col xs={24} md={12}>
              <Form.Item
      ...
      >
        <Form form={categoryForm} layout="vertical" onFinish={handleCreateCategory}>
          <Form.Item label={t('products.categoryName')} name="name" rules={[{ required: true, message: t('validation.required') }]}>
            <Input placeholder={t('products.categoryNamePlaceholder')} />
          </Form.Item>
          <Form.Item label={t('products.categoryNameAr')} name="name_ar" rules={[{ required: true, message: t('validation.required') }]}>
            <Input dir="rtl" placeholder={t('products.categoryNameArPlaceholder')} />
          </Form.Item>
          <Form.Item
            ...
            rules={[
              {
                validator: (_, value) =>
                  isValidBase64ImageDataUri(value)
                    ? Promise.resolve()
                    : Promise.reject(new Error(t('products.categoryImageInvalid'))),
              },
            ]}
          >
            <Base64ImageUpload
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
            <Button onClick={() => { setCategoryModalOpen(false); categoryForm.resetFields(); }} style={{ marginRight: 8 }}>
              {t('common.cancel')}
            </Button>
            <Button type="primary" htmlType="submit" loading={creatingCategory} style={{ background: '#6B1A1A' }}>
              {t('common.create')}
            </Button>
          </Form.Item>
        </Form>
